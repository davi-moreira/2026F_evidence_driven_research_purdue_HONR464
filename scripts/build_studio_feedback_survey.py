#!/usr/bin/env python3
"""build_studio_feedback_survey.py — the Qualtrics studio-feedback instrument.

ONE survey serves every Studio of the course. The student names the studio on
the first question, so the course page carries one link and the export is one
file, no matter how the reading schedule changes.

D57 (2026-08-23) moved this instrument from ONE RESPONSE PER CHAPTER to ONE
RESPONSE PER STUDIO. Forty responses a semester was an instrument competing with
the work it was meant to improve; twelve arrive at the rhythm of the milestones,
close the same Sunday night, and still reach the author before the next studio
starts. The cost is resolution: a studio-level report names a chapter inside its
anchor rather than being filed under one, so the anchor prompts now ask for the
chapter by name.

The studio list is GENERATED from planning/MEETING_SCHEDULE.csv and the chapter
list from the book manifest, exactly like the schedule page: the survey can
never offer a studio the calendar does not run, and never miss one it does.
...
verified and merged here. Three of its rulings changed the design:
  ...
  * ITEM-SPECIFIC RESPONSE OPTIONS, not a repeated agree/disagree grid. In a
    ...
    failure mode of a form answered over and over.
...
Outputs (all in surveys/):
    qualtrics_studio_feedback.txt   Qualtrics "Advanced Format" import file
    studio_feedback_instrument.md   the instrument, its rationale and its sources
...
SPINE = [
    ("familiarity", "Prior topic familiarity",
     "Before this studio's reading, how familiar were you with what these "
     "chapters cover?",
    ...
    ("explain", "Perceived explainability",
     "Right now, how well could you explain this studio's main idea to a "
     "classmate?",
    ...
    ("effort", "Invested mental effort (Paas 1992)",
     "How much mental effort did you invest in understanding this studio's "
     "chapters? Mental effort means how much thinking work you put in, not how "
    ...
    ("reread", "Clarity breakdown",
     ...
     "not yet clear?",
     ["Never", "Once", "A few times", "Many times", "Through most of the reading"],
     "0–4"),
    ("useful", "Perceived research utility",
     "How useful does this studio's reading seem for doing or judging research?",
    ...
    ("ready", "Application readiness",
     "Without any more explanation, how ready are you to try these chapters' "
     "\"It is your turn\" tasks?",
     ["Not at all ready", "Slightly ready", "Moderately ready", "Very ready",
      "Fully ready", "I did not reach those sections"],
...
#: place in this studio's reading, named by chapter, plus its consequence — so
#: the four are comparable as evidence even though they are not interchangeable
#: as items. Each carries a fallback clause so "nothing to report" still
#: produces a located answer.
ANCHORS = [
    ("A", "anchor_a",
     "Point to one specific place in this studio's reading that helped your "
     "understanding. Name the chapter, then the sentence, example, figure, or "
     "step, and say what it helped you see. If nothing helped, name where you "
     "first got stuck and say why."),
    ("B", "anchor_b",
     "Point to one specific place in this studio's reading that slowed you "
     "down or blocked you. Name the chapter, then the sentence, example, "
     "figure, or step, and say what needs to change. If nothing blocked you, "
     "name the hardest place and say what made it work."),
    ("C", "anchor_c",
     "Choose one place in this studio's reading you would revise. Name the "
     "chapter, say where it is, and say exactly what you would change. If you "
     "would change nothing, name one place that should stay exactly as it is, "
     "and say why."),
    ("D", "anchor_d",
     "Write one question this studio's reading left you with, and name the "
     "chapter and the passage that prompted it. If you have no open question, "
     "write a check-for-understanding question for a classmate, and give the "
     "answer."),
]

ANCHOR_INSTRUCTION = ("Answer in one to three sentences. Name the chapter, and "
                      "be specific enough that your instructor can find the "
                      "passage. Longer answers earn no more credit than short "
                      "exact ones.")

INTRO = (
    "This takes about three minutes. You are reporting on ONE studio — the "
    "chapters you were assigned for it. Answer from the reading, not from class."
    "<br><br>"
    "<b>Three things worth knowing before you start.</b>"
    "<br><br>"
    "<b>One.</b> This is scored on whether you submitted a real, specific "
...
def build_txt(studios: list[str]) -> str:
    ...
    p.append(q("MC:DropDown", "studio",
               "Which studio are you reporting on?", choices=studios))
    p.append("[[PageBreak]]\n")
    ...
    p.append(q("MC:SingleAnswer:Vertical", "coverage",
               "How much of this studio's assigned reading did you actually "
               "read? This answer is never scored. Answer it honestly, so the "
    ...
    p.append(q("MC:SingleAnswer:Vertical", "notebook",
               "Did you use the companion Colab notebooks for these chapters?",
               choices=["No, I did not open them", "I opened them but did not run them",
                        "I ran them as written", "I ran them and changed things"]))
    p.append(q("MC:SingleAnswer:Vertical", "notebook_effect",
               "If you used them: what did the notebooks do for your understanding?",
    ...
    p.append(q("MC:SingleAnswer:Vertical", "ai_used",
               "Did you use an AI tool while reading for this studio?",
               choices=["No", "Yes, to summarise or explain part of it",
                        "Yes, to help with an \"It is your turn\" task",
                        "Yes, something else"]))
    ...
    p.append(q("TE:Essay", "defect_where",
               "If you ticked anything: which chapter, where in it, and what "
               "is wrong?"))
...
def build_md(studios: list[str]) -> str:
    ...
    n = len(studios)
    listing = "\n".join(f"| **{i}** | {t} |" for i, t in enumerate(studios, 1))
    return f"""# Studio Feedback Survey — the instrument

*Generated by `scripts/build_studio_feedback_survey.py`. Do not hand-edit;
edit the script.*

One Qualtrics survey covers all **{n}** Studios of the course. The student names
the studio on the first page, so the course page carries **one link** all
semester and the export is **one file** with a `studio` column.

**Import it:** Projects → Create project → Survey → **From a file** → upload
`qualtrics_studio_feedback.txt`.

**When each response is due:** the Sunday 11:59 PM that ends the studio week —
the night before the next studio starts, and the same moment that studio's
milestone is due. The dated list is
...
rotating the closed measures would leave too few observations per studio to
read, and any apparent difference between studios would partly be an artifact
of having asked different questions.

**One response per studio, not one per chapter (D57).** Forty responses a
semester turned a diagnostic into a workload, and a workload gets satisficed.
Twelve ride the rhythm the course already has: the studio closes, the milestone
goes in, the report goes in with it. The trade is real and worth naming — a
studio-level response cannot be filed under a single chapter, so every anchor
prompt now asks the student to name the chapter inside the answer, and the
`defect` channel asks which chapter too.

---

## 1. Identification

`student` (Purdue username) and `studio` (dropdown, generated from the meeting
schedule and labelled with the chapters that studio assigns, so it can never
drift from what the course actually runs).
...
form a latent "studio quality" scale. Do not compute a reliability
...
Every version demands the same thing — a specific place in the studio's
reading, named by chapter, plus its consequence — and every version carries a
fallback clause, so "nothing to report" still produces a located answer instead
of a blank. What rotates is the intellectual act: notice, diagnose, decide, or
ask.
...
`coverage` — how much of the studio's reading you actually read. **Never
scored, and the question says so on the page.** A self-report of effort that
carries a penalty measures fear, not effort. Unscored, it tells the author which
studios get skimmed, which is the single most useful thing a reader can report
about a book still being written.

`notebook` and `notebook_effect` — the companion Colab notebooks are separate
artifacts from the chapters and can fail on their own. A notebook cannot be
credited or blamed when it was never opened, so exposure is asked before effect.
Note the scope: these ask about the BOOK's companion notebooks, not the lecture
notebooks, which are not collected (D57).
...
`defect` (multi-select) and `defect_where` (conditional, and it asks which
chapter). This is the erratum queue: typos, wrong numbers, dead links,
unreadable figures, terms used before they are defined. **Inspect any specific factual, broken-resource or
...
1. **Rotate the anchor.** Block "Your evidence anchor" → *Question
   ...
   globally; it does **not** guarantee one of each per studio, so do not
...
5. **Optional: carry the studio in the link.** Survey Flow → *Embedded Data* →
   `studio_id`, then post per-studio links `…?studio_id=s05` and hide the
...
**The credit ledger** — `student`, `studio`, timestamp, and whether the anchor
names a real place in that studio's reading. This is the only view grading
touches. Scoring is in
[`participation_grading.md`](participation_grading.md); the twelve deadlines are
in
[`../planning/STUDIO_FEEDBACK_SCHEDULE.md`](../planning/STUDIO_FEEDBACK_SCHEDULE.md).

**The revision view** — studio, the chapter named in the anchor, the closed
items, `prompt_version` and the anchor text, notebook answers, and defect flags,
with the student column dropped. This is procedural separation, not anonymity: in a five-person
seminar, a writing style or a referenced experience can still identify someone.
Treat it accordingly.

### Reading it per studio
...
studio when **two or more** students report weak explainability, repeated
...
| Several students name the **same** chapter in the anchor | That chapter is carrying the studio's damage. Read it first. |
...
different studios, anchors at the character minimum, sudden completion-time
...
def studio_choices() -> list[str]:
    ...
    The label carries the chapters so the dropdown itself tells the student
    which reading the response is about; the chapter set comes from the same
    home anchors that set the reading deadlines, so it cannot drift from the
    schedule page.
    """
    index = lesson_index()
    ...
    for r in meetings:
        ...
        if not m:
            continue
        st = studios.setdefault(int(m.group(3)),
                                {"title": m.group(2), "chapters": []})
        st["chapters"] += at.get(int(r["meeting"]), [])

    out = []
    for k in sorted(studios):
        nums = sorted(studios[k]["chapters"])
        span = (f" (Ch. {nums[0]}-{nums[-1]})" if len(nums) > 1
                else (f" (Ch. {nums[0]})" if nums else ""))
        out.append(f"{studios[k]['title']}{span}")
    return out
...
def main() -> None:
    ...
    (OUT / "qualtrics_studio_feedback.txt").write_text(build_txt(studios))
    (OUT / "studio_feedback_instrument.md").write_text(build_md(studios))
    print(f"  ✓ surveys/qualtrics_studio_feedback.txt ({len(studios)} studios, "
          f"{len(SPINE)} spine items, {len(ANCHORS)} anchor versions)")
    print(f"  ✓ surveys/studio_feedback_instrument.md")
