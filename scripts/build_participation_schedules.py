...
def lecture_notebook_page(meetings: list[dict]) -> str:
    ...
    # D79 retired the category, so the weight no longer lives under assessment:.
    # The kept contract block carries what it was, and `retired_this_edition`
    # switches the page's banner. Restoring `lecture_notebooks: 20` under
    # assessment: is what turns the contract back on.
    retired = bool(block.get("retired_this_edition"))
    weight = float(cfg["assessment"].get("lecture_notebooks",
                                         block.get("retired_weight", 20)))
    ...
    banner = "" if not retired else f"""
> **⚠️ RETIRED FOR THIS EDITION (D79, 2026-09-04). Nothing below is collected.**
>
> D74 made the weekly lecture notebook a **{weight:g}%** completion contract on
...
> Project**, which is **{float(cfg["assessment"]["final_project"]):g}%** of the
> grade.
>
...
def lab_meeting_table(meetings: list[dict]) -> str:
    ...
    _ln = _config()["lecture_notebooks"]
    weight = _config()["assessment"].get("lecture_notebooks",
                                         _ln.get("retired_weight", 20))
