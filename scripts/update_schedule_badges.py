...
**Slides column.** Each **Deck** is that Studio's lecture deck, built from the
Studio's own [EDR|AI](book/index.html){target="_blank"} chapters, so it always
says what the book says. Weeks 11 to 14 are the conference block and carry no
Studio, so those rows carry no deck.

**Notebook column.** Every notebook loads the same course dataset bundle:
...
#: under every row's Colab badge; that is 43 copies of a link to a SINGLE file,
#: and the page is held to a 65,535-character ceiling. It is stated once, in
#: the footer directly under the table, where it costs a student nothing.
DATA_ZIP = "[data.zip](notebooks/data/data.zip)"


#: The Slides cell: the studio's lecture deck, generated by
#: scripts/build_studio_slides.py from that studio's own EDR|AI chapters.
#: Weeks 11-14 are the conference block (D50) and anchor no studio, so their
#: rows carry no deck. The link is deliberately short: this page is held to a
#: 65,535-character ceiling and the column repeats on every row.
def slides_cell(studio_rank: int | None) -> str:
    if studio_rank is None:
        return ""
    return f"[Deck](lecture_slides/studio{studio_rank:02d}.html)"
...
def week_studio(unit: str, studios: dict[int, dict],
                first: bool = True) -> tuple[str, str, int | None]:
    """`unit` -> ("Week 4", the Studio cell, the studio rank), per row.
    ...
    rank, st = next(((k, v) for k, v in studios.items()
                     if v["title"] == label), (None, None))
    if st:
...
def build() -> str:
    ...
    lines.append("| # | Date | Week | Studio | Topic | Slides | Notebook "
                 "| Milestone | Required reading |")
    lines.append("|---|------|------|--------|-------|--------|----------"
                 "|-----------|------------------|")
    ...
    for r in rows:
        ...
        week, studio, studio_rank = week_studio(r["unit"], studios,
                                                first_of_week)
        deck = slides_cell(studio_rank)

        for iso in [d for d in sorted(breaks) if d < r["date"]]:
            label = breaks.pop(iso)
            d = date.fromisoformat(iso)
            unit = unit_by_isoweek.get(d.isocalendar()[:2], r["unit"])
            bweek, bstudio, _ = week_studio(unit, studios, False)
            lines.append(
        ...
        lines.append(
            ...
            f"| {studio} | {title} | {deck} | {badge} | {mile} | {materials} |"
        )
