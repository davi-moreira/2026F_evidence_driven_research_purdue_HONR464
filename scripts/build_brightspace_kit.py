...
# role. Every Mon/Wed lecture now opens with a ten-minute LAB MEETING in which
# one student reports a decision from their OWN project, and the instructor
# leads from minute 10. The 25% category is replaced by Lecture Notebooks (20%),
# and the 5 points that were freed went to Milestone Deliverables inside the
# Final Project, which rose from 50% to 55%.
#
...
# The two checklist entries, in their live SRL form and in the lab-meeting form
# that replaced them. The draw itself carried over unchanged (D74 Ruling 6), so
# the disabled variants still publish it — the same dates, a different job on
# the day. The generated files keep their `srl` names because nothing is
# deleted or renamed.
...
LAB_MEETING_CHECKLIST_ANNOUNCEMENT = (
    "`03_srl_slots_and_logistics.md` (after the draw — generated by\n"
    "      `scripts/build_srl_packet.py`, so fix the generator, never the file; FERPA,\n"
    "      course platform only). D74 retired the Student Research Lead role, so what\n"
    "      this one publishes is the **lab-meeting reporter** schedule: the same draw\n"
    "      and the same dates, a different job on the day. The files keep their `srl`\n"
    "      names because D74 deletes nothing. Send each student their\n"
    "      `_adm/roster/srl_packet/per_student/*.md` summary alongside the third."
)
...
LAB_MEETING_CHECKLIST_ASSIGNMENT_TODO = (
    "- [ ] **Post the lab-meeting reporter schedule** so each student sees their own\n"
    "      dates (`_adm/roster/2026F_HONR46400_srl_assignment.md` — the file keeps its\n"
    "      D69/D71 name; D74 changed the job, not the draw). FERPA: post each\n"
    "      student's slots to that student, or post a slot table by date that names\n"
    "      only the reporter for that date — never distribute the roster file itself."
)
...
def gradebook_spec(config: dict) -> str:
    ...
    lines += [
        ...
        "the answers came out right and never on how that week's lab-meeting "
        "report went. Credit 1.0 on time, 0.5 within seven days, 0 after that; "
