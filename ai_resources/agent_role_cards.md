...
M6, M8, M10, and M16 consults); each of those cards names its milestone. The other
...
- **When to consult:** whenever you are about to cite something, and before any source enters your evidence map (M3).
...
- **When to consult:** at M4 (the inquiry declaration) and any time you revise the question.
...
- **When to consult:** at M4 and whenever a design element changes.
...
- **When to consult:** M5, when building an observational descriptive design.
...
### Causal Identification Skeptic *(GenAI Studio custom model — M6 touchpoint)*
- **Purpose:** the hostile causal reviewer. Attacks every reason your claimed effect might not be identified.
- **When to consult:** M6, the required touchpoint for observational causal work, and any time you use a causal verb.
...
### Prediction & Leakage Auditor *(GenAI Studio custom model — M8 touchpoint)*
- **Purpose:** for predictive work, hunts for leakage and dishonest evaluation, and checks the held-out and baseline are real.
- **When to consult:** M8, the required touchpoint for prediction, and any predictive claim.
...
### Robustness & Sensitivity Reviewer *(GenAI Studio custom model — M10 touchpoint)*
- **Purpose:** probes whether your result survives reasonable alternative choices, and where it is fragile.
- **When to consult:** M10, alongside the Poster Critic, and whenever you finalize a result.
...
### Poster Critic *(GenAI Studio custom model — M10 touchpoint)*
- **Purpose:** the 90-second stranger. Attacks the poster's headline, figure honesty, read path, and visible uncertainty.
- **When to consult:** M10 draft review and before the final lock (M13).
...
### Reproducibility Auditor *(GenAI Studio custom model — M16 touchpoint)*
- **Purpose:** checks whether your package (or a peer's) can be rebuilt by a stranger, and traces numbers from data to claim.
- **When to consult:** M16 replication and red-team, and when assembling your capsule (M16).
...
- **When to consult:** M16, when drafting the note and brief.
