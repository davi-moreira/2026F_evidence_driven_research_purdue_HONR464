...
- Three parts: the sentence, the card that pins its terms, and the boundary pair.
- Kind and reach come straight from the compass; the boundary is written now, not after the results.

![](figs/declaration_anatomy.png){fig-align="center" width="80%" fig-alt="The declaration, drawn as the record it is. The lead question is one part and the field card that pins every term it carries is the other; the claim b"}

::: {.figure-caption}

The declaration, drawn as the record it is. The lead question is one part and
the field card that pins every term it carries is the other; the claim
boundary is a pair, written now rather than after the results. The band
beneath is the line no tool crosses.
...
in, not a sentence to polish. Two things repay pointing at. The boundary is a
pair, and both halves are written before any results exist, which is what
makes the second half a promise rather than an excuse. And the band underneath
is the delegation line for this whole lesson: candidate wording is the part
that delegates well, while the choice among the wordings, the kind, the reach
and both boundary sentences stay with the researcher.
