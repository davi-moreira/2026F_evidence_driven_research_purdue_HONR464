...
## A result you cannot reproduce is a rumor {data-menu-title="Ch 4 · A result you cannot reproduce is a rumor"}
...
and measured against what? A result I cannot reproduce is a rumor.
...
pages, I expect the new page to win by a few percent, not by forty, and both
versions have to be measured over the same weeks. Interrogate sounds like
this: when you report turnout, is the denominator registered voters or every
...
- Interrogate: the tool admits it combined every session, from both weeks.
...
two versions have to be measured over the same weeks, on the same kind of
traffic. Delegate was the script that computes each version's rate from the
...
## Two numbers describe the same gap {data-menu-title="Ch 4 · Two numbers describe the same gap"}
...
[Percentage-point difference]{.t}

[One rate minus the other. 4.24 percent minus 4.00 percent is 0.24 percentage points.]{.d}
...
[That difference against the rate you started from. 0.24 divided by 4.00 is 6 percent.]{.d}
...
Both describe one thing: twelve more recorded purchases for version B out of
5,000 visits. Ask the room which of the two a headline would print, and why.
This is the confusion that makes a small gap sound decisive, and naming both
scales is the cure.

:::


## The log keeps its exact counts {data-menu-title="Ch 4 · The log keeps its exact counts"}
...
The first block builds visit-level rows from three sets of exact counts, under
SEED = 464. Inside each version-week group it shuffles the purchase and
non-purchase rows without changing either count, so the rates are fixed by the
counts alone. Note what the log does not hold: no routing, no shopper
identity, no day.

:::


## One gap, reported on both scales {data-menu-title="Ch 4 · One gap, reported on both scales"}
...
- All weeks: 3.05 percent against 4.24 percent, a 39 percent relative increase.
- Week 2 alone: 4.00 percent against 4.24 percent, 0.24 points, 6 percent relative.
...
Have the room predict the all-weeks figure before you run it, then read the
two results against each other. The first is what the tool's script produced.
The second is what survived Inspect and Verify. Restricting to the shared week
removes one unfair difference between the groups. It does not turn what is
left into a finding.

:::


## A higher recorded rate is not yet a performance claim {data-menu-title="Ch 4 · A higher recorded rate is not yet a performance claim"}
...
- Verify: restrict both versions to week 2 and recompute.
- Also confirm both versions count a purchase the same way.
- Document: the comparison thrown away, the one kept, and both scales.
- Defend: 4.00 against 4.24 in the one shared week, and what the log cannot settle.

:::::

::: {.notes}

The defended sentence in full: "in the one week both versions were live, the
old page converted 4.00 percent of visits and the new page 4.24 percent. That
is 0.24 percentage points, a 6 percent relative increase, and twelve
purchases. This log does not record how visits reached either version or
whether shoppers came back, so it does not establish that the new page
performs better. I have not tested it during a sale, and I have not looked at
mobile traffic separately." Running a fixed checklist rather than trusting
recall is an old and well-studied safeguard in high-stakes work (Degani &
Wiener 1993). The tool did the typing. You did the research.
...
checkout page converts 39 percent better." The arithmetic is right and the
code runs without a single error. The comparison is still wrong. Here is the
trap: the script combined every session in the log. The old page's numbers
include the sale week, when the site was full of bargain hunters who never
intended to buy, and the new page only existed afterward. The comparison
charged one version a crowd the other never met.
...
- Reading the script shows it never requires both versions to appear in the same week.
- Restricting the comparison to the week they actually ran side by side shrinks the 39 percent to 6.
- That repair removes one unfair difference between the two groups.
- It does not turn what is left into a finding, because the log still cannot say how a visit reached either version.

:::::

::: {.notes}

You catch it at Inspect and Verify. Reading the script shows it never requires
both versions to appear in the same week. Restricting the comparison to the
week they actually ran side by side shrinks the 39 percent to 6. That repair
removes one unfair difference between the two groups. It does not turn what is
left into a finding, because the log still cannot say how a visit reached
either version. A green check is not a correct result, and a corrected number
is not automatically a claim you can defend. You verify the comparison and the
number, not the paragraph about them.
...
## The whole declaration, on one card {data-menu-title="Ch 7 · The whole declaration, on one card"}
