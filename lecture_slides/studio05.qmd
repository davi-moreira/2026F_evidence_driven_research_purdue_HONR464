...
## The lessons in this studio {.densest}
...
## Observational Descriptive Research {background-color="#1a1a19" .divider}
...
## The frame crosses the edge of your target, and that is where errors live {.dense}
...
![](figs/sampling_groups.png){fig-align="center" width="52%" fig-alt="The four groups a description has to keep straight. The frame crosses the edge of your target population, which is where the three coverage errors liv"}

::: {.figure-caption}

The four groups a description has to keep straight. The frame crosses the edge
of your target population, which is where the three coverage errors live:
people inside the target who never reach the list, records on the list that
fall outside the target altogether, and one unit listed twice.
...
## Build the whole customer base, let the worst-hit quit, then survey {.denser}
...
## Ten honest samples and one convenience sample, all of size 500 {.denser}
...
## Same sample size, about sixteen years off
...
![](figs/ch11_sampling_channels.png){fig-align="center" width="58%" fig-alt="Ten random sample means cluster around the true mean age; the single convenience-channel mean sits far below it."}

::: {.figure-caption}

Ten random sample means cluster around the true mean age; the single
convenience-channel mean sits far below it.
...
## Observational Causal Research {background-color="#1a1a19" .divider}
...
## Build mice where the microbe does nothing, then read the gap {.densest}
...
## Experimental Descriptive Research {background-color="#1a1a19" .divider}
...
## An experiment can be an instrument rather than an intervention {.dense}
...
## Demand and instrument effects both threaten construct validity {.dense}
...
## Run it: the randomization is real, the claim it licenses is still descriptive {.denser}
...
## Prediction and Generalization {background-color="#1a1a19" .divider}
...
## Three summers in code, and the last one stays locked {.densest}
...
## Twelve polynomials, chosen on selection, scored once {.densest}
...
## Only data the fit never touched can tell you to stop at six {.dense}
...
![](figs/ch14_overfitting.png){fig-align="center" width="72%" fig-alt="Left: training error keeps falling while selection error turns up after degree six, where the model is chosen; a diamond marks its one final-holdout s"}

::: {.figure-caption}

Left: training error keeps falling while selection error turns up after degree
six, where the model is chosen; a diamond marks its one final-holdout score.
Right: across 500 worlds, the winner's true error exceeds its crowning score
on average, with a long right tail.
...
## Experimental Causal Research {background-color="#1a1a19" .divider}
...
## Run the trial twice: everyone randomized, then only those measured {.densest}
...
## Two thousand coin flips over the same 200 patients {.densest}
...
## The estimates center on the truth, and they still wobble
...
![](figs/ch15_randomization.png){fig-align="center" width="70%" fig-alt="The histogram of 2,000 re-randomized estimates centers on the true 5-point effect."}

::: {.figure-caption}

The histogram of 2,000 re-randomized estimates centers on the true 5-point
effect.
...
## Keep the coin flip honest and change only who is measurable {.denser}
...
## Not one of 2,000 re-randomizations landed near the truth
...
![](figs/ch15_attrition.png){fig-align="center" width="70%" fig-alt="The complete-case contrasts pile up near 8 points, far from the true 5-point effect."}
...
## Hybrid and Complex Designs {background-color="#1a1a19" .divider}
...
## Run the index both ways and watch the wrong number hold still {.denser}
...
## Milestone 5: Your pathway, declared {background-color="#1a1a19" .divider}
