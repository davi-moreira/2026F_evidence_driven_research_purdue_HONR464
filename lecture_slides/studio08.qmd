...
## Check that every row answers the same question before reading a number {.densest}
...
## Three tests aim at zero, and the vehicle control matches your solvent {.dense}
...
## Both controls get read before the extract's ring means anything {.densest}
...
## Build a world where the vehicle does nothing, then run it 2,000 times {.denser}
...
## Watch which line decides, not which reviewer sounded surest {.densest}
...
## Recomputing the drop takes about as long as reading the confident sentence {.denser}
