...
![](figs/mida_map.png){fig-align="center" width="58%" fig-alt="Diagram with four rounded boxes in two rows. Top row, labeled what you want to learn: Model, how the world could work, with an arrow labeled makes askable pointing to Inquiry, the one quantity you want from it. Bottom row, labeled how you will learn it: Data strategy, every procedure that makes the data including sampling, assignment and measurement, with an arrow labeled feeds pointing to Answer strategy, how those data become an answer with uncertainty included. A dashed arrow runs upward from Answer strategy to Inquiry, labeled alignment: does the answer reach the inquiry?"}
...
![](figs/diagnose_loop.png){fig-align="center" width="62%" fig-alt="Three boxes left to right, Declare, Diagnose, and Redesign, joined by arrows, with a return line running from Redesign back to Declare beneath them, labeled one change at a time. Above the middle box: read out, bias, wobble, how often it detects. A wide box beneath reads: the honest call, run it as declared, redesign again, or narrow the claim to what this design can deliver."}
