...
|---|---|---|---|---|
| world/PO simulation pattern | declaration-style model (ch. 6–7 concept) | nb04 inline | known-truth check (§3) | 🔜 pending — nb04 self-check cells (flips ✅ at the Phase F execute-all audit) |
| complete random assignment | randomizr `complete_ra` | nb07/nb11 inline | structural + balance check | 🔜 pending — nb07 self-check cells |
| difference-in-means + SE | estimatr `difference_in_means` | nb09/nb13 inline | real-data check on foos_etal (§4) | 🔜 pending — nb13 self-check cells |
| OLS + HC2 | estimatr `lm_robust` | nb09 inline | cross-check vs classical SEs + statsmodels docs | 🔜 pending — nb09 self-check cells |
| Monte-Carlo diagnosis (bias/power/coverage) | `diagnose_design` (ch. 10) | nb10/nb11 inline | tolerance vs analytic power (§2–3) | 🔜 pending — nb10/nb11 self-check cells |
| redesign grid | `redesign` (ch. 11) | nb11 inline | monotonicity checks (power ↑ with n) | 🔜 pending — nb11 self-check cells |
| power simulation (PS1 concept) | rdss-problem-set-1 | nb10 inline | tolerance vs `statsmodels.stats.power` | 🔜 pending — nb10 self-check cells |
| DAG drawing | make_dag_df.R | nb04 inline | structural (edge list) only | 🔜 pending — structural check in nb04 (visual aid, no numbers) |
