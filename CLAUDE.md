...
| `surveys/participation_grading.md` | Anything about the Participation 9% or the IYT Practice 15% — the **two** undivided completion contracts (D57, amended D58/D61). A third, **Lecture Notebooks 20%**, arrived with D74 and was **retired by D79**; its section is kept there behind a banner |
...
| `_adm/student_research/2026Fall/README.md` | Private 2026F student-submission intake, milestone reviews, mentoring emails, and longitudinal research-progress records |
| `planning/LECTURE_NOTEBOOK_SCHEDULE.md` | **Retired by D79 — kept as a record.** The 16 weekly Lecture Notebooks deadlines D74 created; still generated by `scripts/build_participation_schedules.py`, never hand-edited, and no longer in force |
| `project/srl/` | **Retired for this edition, kept in full for a future one (D74)** — Student Research Lead handbook, templates, rubric; never delete any of it. The per-lecture questions guide is kept and re-owned to the instructor |
...
## 🚨 CRITICAL WORKFLOW — Milestone Grading & Research Mentoring  *(2026F)*

When Davi says that HONR 464 submissions have been downloaded, or asks to
grade/review a student's milestone or draft the student's mentoring email, use
...
1. **Grader feedback (milestones only):** apply the exact milestone rubric and
   hard caps, produce concise Brightspace-ready feedback, and distinguish
   evidence visible in the submitted artifact from live/classroom or platform
   evidence that Davi must check. Never infer a pitch, timestamp, separate
   attachment, or completed ledger that is not present.
2. **Research-mentor email:** save the full message as Markdown and reproduce it
   in the chat. Give individualized, concrete scientific advice that improves
   the next milestone and keeps the project on a feasible path to the poster.
   Use only real, independently verified publications or authoritative research
...
truncated, missing, or illegible submission remains pending; request a clean
copy rather than inventing evidence or a score.
...
- **Wednesday:** 0–10 lab meeting · 10–30 intensive applied AI laboratory ·
  ...
  untouched, and with the lead role retired the synthesis is the instructor's) ·
...
**Section boundaries 3 and 4 are UNCHANGED** (31–43 / 43–50 Monday, 30–42 / 42–50
Wednesday), so every D22 and D34 ruling about the later blocks stands exactly as
written. Friday's 5 / 40 / 5 studio frame (D58) is untouched.
...
**Never reintroduce, in any surface:** lecture-notebook completion **as a participation
item** (D74 collected the lecture notebooks weekly under their own 20% contract and D79
retired that contract, so they are not collected at all — either way they are never
participation), per-chapter reading feedback, the retired
...
```bash
.venv/bin/python scripts/build_participation_schedules.py   # IYT + studio + lecture-notebook + lab-meeting tables
...
## 🚨 CRITICAL RULE — Nothing From a Lecture Is Collected  *(D79 — retires D74's Lecture Notebooks contract)*

**There is no Lecture Notebooks grade category.** D74 made the weekly notebook a
20% completion contract on 2026-08-31; **D79 (2026-09-04) retired it**. Nothing
from a Monday or Wednesday lecture is collected or graded: not the notebook, not
the ungraded lab-meeting round (D75). **The weekly notebook is still worked in
class and is still the lesson** — students open it, work it, and keep it. It is
simply never a submission, and no surface may say otherwise.

**D74's authoring constraints survive as PEDAGOGY, not as a grading rule.** Every
writing move still leaves a visible, fillable place; each lecture's required path
still fits its 50 minutes; material below a lecture's `### ⏸` line or behind a 🏠
label is still optional depth. What is gone is the sentence that made those a
*credit*.

**DELETE NOTHING — the retirement is IN PLACE**, the fourth time this rule fires
(after D58's quiz banks and D74/D75's SRL suite). Kept on disk, unapplied:
`course_config.yaml`'s whole `lecture_notebooks:` block (now carrying
`retired_this_edition: true` and `retired_weight: 20`),
`planning/LECTURE_NOTEBOOK_SCHEDULE.md` behind its banner, the generator in
`scripts/build_participation_schedules.py`, every category string in
`scripts/build_brightspace_kit.py` behind
`LECTURE_NOTEBOOK_CATEGORY_ENABLED = False`, and the commented-out paragraph,
weights row and numbering entry in `scripts/build_syllabus_docx.py`. Reinstating
costs: restore `lecture_notebooks: 20` under `assessment:`, put `final_project:`
back to 55, clear `retired_this_edition`, flip the flag, rerun the two generators.

**The contract as D74 wrote it, kept as the record:**

> **Lecture Notebooks is 20%**, a top-level assessment category and the course's
> **third undivided completion contract**. It holds exactly one item family: the
> weekly notebook `nbNN` worked in class. It took the place of the retired 25%
> Student Research Lead category.
>
> - **One submission per week per student** — the notebook worked in class,
>   nb01–nb16.
...
>   `planning/LECTURE_NOTEBOOK_SCHEDULE.md` (generated by
>   `scripts/build_participation_schedules.py`; never hand-edited).
>
> This work is **not participation** and must never be called participation on any
> surface. The ±0.9-point contribution adjustment belongs to the 9% block alone and
> never touches this one. D57's ban survives in amended form: lecture-notebook
> completion may never return **AS A PARTICIPATION ITEM**.

**The freed 25 points.** `1 + 9 + 15 = 25`, so all 25 points freed by retiring
the Student Research Lead are forced inside the Final Project, which **rises
55% → 75%**. Inside it the weight goes **mostly to Milestone Deliverables, which
rises 20 → 33 course points**, because those points came off WEEKLY COLLECTED
WORK and M1–M16 is the weekly deliverable chain that remains; the other four rise
10 → 12, 5 → 6, 10 → 12, 10 → 12.

This **AMENDS the seventh documented deviation** from D53's verbatim QM 47400
adoption rather than opening an eighth — the deviation is still "the shares are
not QM 47400's", only the numbers move. Student-facing surfaces print **course
percentages**: Milestone Deliverables 33% · Peer Evaluation 12% · Peer Review 6% ·
Poster Presentation at the Purdue Undergraduate Research Conference 12% ·
Instructor/TA Evaluation 12% (= 75% of the final grade). `course_config.yaml`
keeps the project shares 44 / 16 / 8 / 16 / 16 (sum 100.00) as the machine record.
Unlike D74's, **both columns terminate exactly** (`75 × 0.44 = 33`,
`75 × 0.16 = 12`, `75 × 0.08 = 6`), so no component carries a rounding remainder.
...
scores. **Participation and IYT Practice are untouched in every number.**

**The weights, and they sum to 100:**
...
**D74 — the SRL material is KEPT, the role and the category are not.** Exactly as
D58 kept the quiz banks after retiring the quiz category, D74 keeps every SRL
artefact on disk for a future edition: all of `project/srl/` (handbook, rubric,
Socratic question bank, AI-integration guide, prep template, peer-feedback form,
submission instructions, absent-lead and instructor-intervention protocols),
`scripts/assign_srl_slots.py` and `scripts/build_srl_packet.py`. **Never delete an
SRL file, an SRL script, or an SRL section.** Retire IN PLACE: mark it clearly as
*retired for this edition, kept for a future one*, say what replaced it, and leave
the content intact. The per-lecture questions guide is kept in full and re-owned
to the instructor. What is retired is the grade category and the live role, never
the material.
...
The decks are graded on nothing, and since **D79** neither is anything else from a
Mon/Wed lecture — the Lecture Notebooks contract D74 created is retired.
...
round** — and the weekly notebook is still worked in class because it **is** the lesson, it is
simply never a submission. All **25 points** freed by retiring the Student Research Lead now
sit inside the **Final Project, which rises 55% → 75%**, and inside it the weight goes
**mostly to Milestone Deliverables (20 → 33 course points)**, because those points came off
WEEKLY COLLECTED WORK and M1–M16 is the weekly deliverable chain that remains; loading the
November one-shots instead would have changed what the course rewards. The other four rise
10 → 12, 5 → 6, 10 → 12 and 10 → 12, so the project shares become **44 / 16 / 8 / 16 / 16**
and the course percentages **33 / 12 / 6 / 12 / 12**. Unlike D74's, **both columns terminate
exactly** — no component carries a rounding remainder. This **AMENDS** D74's seventh deviation
from D53's verbatim QM 47400 adoption rather than opening an eighth: the deviation is still
"the shares are not QM 47400's", only the numbers move. **No component was renamed and no
scoring rule inside a component changed**, and **Participation (9%) and IYT Practice (15%) are
untouched in every number** — same N, same d, same credit rule, same formulas, and the ±0.9
contribution adjustment still belongs to the 9% alone. D57's ban survives in D74's amended
form: lecture-notebook completion may never return **AS A PARTICIPATION ITEM**. Weights:
attendance 1 · participation 9 · IYT Practice 15 · Final Project 75 (= 100). **DELETE NOTHING
— the fourth time this rule fires** (after D58's quiz banks and D74/D75's SRL suite): the
whole `lecture_notebooks:` block in `course_config.yaml` (now
`retired_this_edition: true`, `retired_weight: 20`), `planning/LECTURE_NOTEBOOK_SCHEDULE.md`
behind its banner, the generator in `scripts/build_participation_schedules.py`, every category
string in `scripts/build_brightspace_kit.py` behind
**`LECTURE_NOTEBOOK_CATEGORY_ENABLED = False`**, and the commented-out paragraph, weights row
and numbering entry in `scripts/build_syllabus_docx.py` all stay on disk, retired IN PLACE.
D74's **authoring constraints survive as pedagogy**, not as a grading rule. ⚠ Action for Davi
OUTSIDE the repository: `_announcements/2026-08-31_srl_retired.md` told students in a table
that lecture notebooks are 20% and are collected weekly — if it was posted, they need the
correction, and `_announcements/2026-09-04_lecture_notebooks_retired.md` is written for it.
The Brightspace gradebook also needs rebuilding from the regenerated
`brightspace/gradebook_spec.md`: its Final Project block is still at QM 47400's original
35%-total shares (10.5 / 3.5 / 7 / 7) and **Peer Evaluation is missing entirely**, which is why
the platform reports the total as 53%.
(7.3 — D78, the studio decks get a real visual system (2026-09-02,
...
spends 7 minutes on a decision from their OWN project plus the evidence behind it, then 3
minutes of questions from the room; the reporter does not teach the concept, the report is
not graded, and **the instructor leads from minute 10**, owning accuracy, the AI tooling and
the clock. Both frames still sum to 50: **Monday 10 / 21 / 12 / 7** and **Wednesday 10 / 20 /
12 / 8**, with section boundaries 3 and 4 unchanged (D22 and D34 stand untouched) and
Friday's 5 / 40 / 5 studio frame (D58) unaffected. In the category's place, **Lecture
Notebooks (20%)** becomes the course's THIRD undivided completion contract: one submission
per week per student (nb01–nb16), due 11:59 PM on the Sunday that ends the studio week (Week
16 closes Fri Dec 11), graded on **completion only** — never on whether the answers came out
right, never on how the live report went — credit 1.0 / 0.5 / 0, N = 16, d = 2, `points =
20.0 × (sum of the highest 14 credits) / 14`. It is NOT participation and never carries the
±0.9 contribution adjustment; D57's ban survives in amended form (never again **as a
participation item**). The freed 5 points are forced inside the Final Project: **Milestone
Deliverables 15 → 20 course points** and the **Final Project 50% → 55%** — the SEVENTH
documented deviation from D53's verbatim QM 47400 adoption, so student-facing surfaces now
print course percentages (20 / 10 / 5 / 10 / 10) while `course_config.yaml` keeps the exact
project shares 36.37 / 18.18 / 9.09 / 18.18 / 18.18 (sum 100.00) as the machine record. No
component was renamed and no scoring rule inside a component changed. **DELETE NOTHING**
(Ruling 5, the parallel to D58's quiz banks): all of `project/srl/`,
`scripts/assign_srl_slots.py` and `scripts/build_srl_packet.py` stay on disk, retired IN
PLACE, and the per-lecture questions guide is kept in full and re-owned to the instructor.
The D69/D71 draw carries over unchanged — 25 slots over **6 students**, 4/4/4/4/4/5 — now
naming reporters rather than leads; no re-draw. Notebook cells: `### 🎤 SRL Lead Brief` →
...
(7.0 — D62, Davi's Word pass folded back into the generator
...
lecture-notebook completion (notebooks were not collected then — **superseded by D74**,
which collects them weekly under Lecture Notebooks 20%; only the participation framing
stays retired), and per-chapter reading
