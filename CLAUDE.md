...
| `surveys/participation_grading.md` | Anything about the Participation 9% or the IYT Practice 15% — two of the three undivided completion contracts (D57, amended D58/D61; it no longer owns "It is your turn" as participation). The third, **Lecture Notebooks 20%**, arrived with D74 |
...
| `planning/LECTURE_NOTEBOOK_SCHEDULE.md` | The 16 weekly **Lecture Notebooks (20%)** deadlines (D74; generated by `scripts/build_participation_schedules.py`, never hand-edited) |
...
item** (D74 collects the lecture notebooks weekly under their own 20% contract, so they
ARE collected — they are simply never participation), per-chapter reading feedback, the retired
...
## 🚨 CRITICAL RULE — Lecture Notebooks Are Their Own 20% Completion Contract  *(D74)*
...
**The freed 5 points.** `1 + 9 + 15 + 20 = 45`, so the remaining 5 are forced
inside the Final Project: **Milestone Deliverables rises 15 → 20 course points**
and the **Final Project rises 50% → 55%**. This is the **seventh documented
deviation** from D53's verbatim QM 47400 adoption — the project-share column no
longer reads 30/20/10/20/20, and because 20/55 does not terminate, student-facing
surfaces now print **course percentages** instead: Milestone Deliverables 20% ·
Peer Evaluation 10% · Peer Review 5% · Poster Presentation at the Purdue
Undergraduate Research Conference 10% · Instructor/TA Evaluation 10% (= 55% of the
final grade). `course_config.yaml` keeps the exact project shares 36.37 / 18.18 /
9.09 / 18.18 / 18.18 (sum 100.00) as the machine record only. **No component was
renamed and no scoring rule inside a component changed** — Milestone Deliverables
is still the equally weighted mean of the M1–M16 milestone scores.
