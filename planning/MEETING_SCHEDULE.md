...
- **Instructor prep:** Verify nb01 loads in Colab from the public repo; rehearse the live Gemini citation-catch and keep an offline screenshot backup; confirm GenAI Studio logins work; roster the 5 students.
...
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: each student reports one change, one blocker, one decision, one thing delegated to AI, and one thing that still needs verification; 15–32 milestone kickoff from the Brightspace brief: present M1 (research opportunity landscape) + rubric, then an AI-supported sprint — 30-second curiosity pitches, then use Gemini to widen each curiosity into candidate problem areas while logging every prompt; 32–42 peer + AI red-team review: classmates and a Gemini skeptic pass each pitch back one 'so what?' and one place the AI over-promised; 42–50 revise, update the Ledger + dossier, submit M0 + Claim Ticket.
...
- **Milestone work time:** 17 min studio block
...
- **50-minute dynamic:** 0–9 SRL-led research puzzle: two students' broad interests side by side, only one already a research problem — the class names what the second added; 9–31 guided Gemini research-partner investigation in nb02: run your curiosity down the funnel, asking Gemini to surface candidate units, populations, and outcomes, then verify each suggestion against a real source; 31–43 human verification + instructor formalization: the instructor formalizes topic vs problem vs question and the importance/feasibility/contribution test, and the class corrects one over-eager AI suggestion; 43–50 decision & defense: log the funnel decisions in the Ledger + Claim Ticket.
- **Hands-on activity:** Run your curiosity through the nb02 funnel to a candidate problem, using Gemini to propose units, populations, and outcomes and verifying each before you keep it.
...
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: each student reports change, blocker, decision, delegation, and what still needs verification; 15–32 milestone kickoff from the Brightspace brief: present M2 (verified evidence + contribution map) + rubric, then an AI-supported sprint — pitch your landscape, pick a lead problem, and use Gemini to surface where evidence for it might live while logging each lead to verify next week; 32–42 peer + AI red-team review: the class and a Gemini skeptic challenge each lead problem's feasibility and compass position; 42–50 revise, update the Ledger + dossier, submit M1 + Claim Ticket.
...
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: change, blocker, decision, delegation, verification-needed from each student; 15–32 milestone kickoff from the Brightspace brief: present M3 (research charter + MIDA declaration) + rubric, then an AI-supported sprint — gallery-walk the evidence maps, and use Gemini to stress-test each stated contribution while logging what you keep; 32–42 peer + AI red-team review: sticky-note challenges plus a Gemini skeptic ask whether each gap is already covered by one of the student's own sources; 42–50 revise, update the Ledger + dossier, submit M2 + Claim Ticket.
...
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: each project states its population, its frame, and the one coverage worry it could not shake this week; 15–32 milestone kickoff from the Brightspace brief + AI-supported sprint: present the M5 deliverable and rubric (a causal identification strategy or an honest causal-language boundary), then run five 3-minute M4 walkthroughs with Gemini-assisted note-taking; 32–42 peer + AI red-team review: two-line peer reviews (best boundary / weakest boundary) plus a Gemini pass that tries to find a population the audit overclaims; 42–50 revise, update ledger + dossier, submit + Claim Ticket: patch the weakest boundary, update the dossier and ledger, submit M4, and write the Claim Ticket.
...
- **Risks / contingency:** A student is stuck alone → nb06 embeds hints behind expandable cells and an office-hours booking link; GenAI Studio unavailable → a provided Gemini fallback skeptic prompt stands in and the required review is logged when the service returns; the board is monitored remotely through the day.
...
- **Risks / contingency:** Survey-experiment analysis confuses → nb07 wraps the estimate in a helper that prints only the measured quantity and its uncertainty; time short → the list-experiment simulation moves to homework.
...
- **Practice:** 📝 nb07 practice: for a calibration protocol, an A/B instrument test, and a conjoint task, name the demand or instrument effect most likely to fake the result and the redesign that defuses it.
...
- **Dataset / simulation:** student's own project data plans (no shared dataset today)
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: each project states the construct it measures and the artifact it most fears; 15–32 milestone kickoff from the Brightspace brief + AI-supported sprint: present the M7 deliverable and rubric (a declared analysis protocol), then run the URC abstract workshop with Gemini-assisted tightening against the uncertainty-and-reach checklist; 32–42 peer + AI red-team review: partner abstract swap plus a Gemini pass hunting the one sentence a skimming reader would over-believe, and a protocol clinic on the worst unaddressed artifact; 42–50 revise, update ledger + dossier, submit + Claim Ticket: verify every abstract claim traces to evidence, submit M6, clear the abstract at the internal gate, update the ledger, and write the Claim Ticket.
...
- **Dataset / simulation:** student's own project data plans (no shared dataset today)
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: each project states its analysis question and the one place leakage could enter; 15–32 milestone kickoff from the Brightspace brief + AI-supported sprint: present the M8 deliverable and rubric (a minimum viable analysis), then assemble the declared analysis protocol from the template; 32–42 peer + AI red-team review: run the required GenAI Studio 'Prediction & Leakage Auditor' review on each protocol and exchange peer red-team notes on the hit each auditor found; 42–50 revise, update ledger + dossier, submit + Claim Ticket: patch the auditor's hardest hit, log the review in the ledger, submit M7, and write the Claim Ticket.
...
- **Other material:** nb09 student notebook (experimental causal research), §1–3 (Colab); Gemini in Colab as the in-class research partner
...
- **Other material:** nb09 student notebook (experimental causal research), §4–6 (Colab); Gemini in Colab as the applied-lab partner
...
- **50-minute dynamic:** 0–7 SRL-led retrieval & challenge: the lead replays yesterday's clean estimate, then breaks it — a third of units vanish before measurement — and challenges the class to say what the number now means; 7–30 intensive applied AI laboratory: in nb09, use Gemini to simulate blocking and clustering (does the interval tighten or widen?), run a factorial cell, then inject attrition, noncompliance, and spillover and watch the estimate move — inspecting and correcting every Gemini step; 30–42 peer defense + adversarial questioning: in pairs, each student defends 'my M8 estimate survives these threats because…' while the partner plays the spillover-and-attrition skeptic; 42–50 transfer to your own project + ledger + Claim Ticket.
...
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: each student's one-line status on their minimum viable analysis (what ran, what is shaky); 15–32 milestone kickoff + AI-supported sprint: present the M9 brief on Brightspace — poster draft 1 plus a research audit — then a supported sprint to finalize the M8 estimate and verify it, Gemini on call for the stubborn cell; 32–42 peer + AI red-team review: 60-second evidence pitches (my estimate, my check, my boundary) on a hard timer, peers and a Gemini skeptic each naming the one thing they do not yet believe; 42–50 revise, log the ledger, submit M8 tonight + Claim Ticket.
...
- **Other material:** nb10 student notebook (attack the analysis), §1–3 (Colab); Gemini in Colab as the robustness-lab partner; project/poster/redteam_review_protocol.md (the four-audit vocabulary, previewed)
...
- **50-minute dynamic:** 0–9 SRL-led research puzzle: the lead shows one result that lives or dies by a single specification choice and asks the class to rule fragile vs cherry-picked; 9–31 guided Gemini research-partner investigation in the notebook: nb10 pre-lists the checks first, then reruns the estimate across alternative samples, measurements, specifications, and metrics, adds placebo and leave-one-out influence cells, and reads the specification curve — Gemini proposing checks, the class inspecting each; 31–43 human verification + instructor formalization: separate the checks that were pre-committed from the ones that would be fishing, and name the honest headline the surviving evidence supports; 43–50 decision & defense: ledger entry + Claim Ticket.
...
- **Practice:** 📝 nb10 practice: label six analyses as robustness check or specification search; for the shared outlier dataset, find the one point whose removal flips the result.
...
- **Homework / next milestone:** Run the rest of your pre-listed checks in nb10; Wednesday you turn AI reviewers loose on your own analysis and compare what they catch.
...
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: each student's one-line status on their poster draft and audit; 15–32 milestone kickoff + AI-supported sprint: present the M10 brief on Brightspace — the final poster lock and the criticism week that leads to it — then a sprint where every student runs the REQUIRED GenAI Studio Poster Critic and Robustness & Sensitivity Reviewer roles against their own draft and logs both in the ledger; 32–42 peer + AI red-team review: trade drafts, apply the four-audit red-team slip (claim boundary, figure honesty, read path, uncertainty), and compare what the peer caught against what the AI roles caught; 42–50 revise, log the ledger, submit M9 tonight + Claim Ticket.
...
- **Other material:** nb11 student notebook (poster criticism and final lock), §1–2 (Colab); project/poster/redteam_review_protocol.md (four-audit + accessibility slip); the milestone_10 poster rubric (the five graded rows, incl. the AI Research Ledger); Gemini for AI-generated skeptic questions per poster
...
- **Other material:** nb11 student notebook (poster criticism and final lock), §3 (Colab); project/poster/redteam_review_protocol.md; a three-lens question deck (interdisciplinary / methods / skeptic); Gemini for generating additional hard questions per poster
...
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: each student's one-line readiness call (what is left before the file locks); 15–32 final production sprint: silent studio with rotating instructor checklist consults — print-spec, figure resolution, uncertainty visible, headline inside its boundary — while each student runs the citation-verification pass (every source retrieved and confirmed) and completes the AI-use disclosure block; 32–42 final-lock checklist audit pairs: partners trade posters and run three audits — citation verification, claim–evidence traceability (one number shown to its notebook cell), and disclosure-block completeness — initialing each; 42–50 lock, submit for printing, close the ledger + Claim Ticket. The deadline is today, 5:00 PM.
...
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: the single most surprising moment at your poster, in one line each; 15–32 milestone kickoff + reflection sprint: the instructor kicks off M13 by handing out the ANONYMIZED replication packages for the Thanksgiving async exchange, then the reflection runs as the work block — code every Expo question into the tally (type, target: claims vs communication, recurrence), find your pattern, and assemble the criticism portfolio from the three posters you evaluated; 32–42 peer + AI red-team review: swap tallies with a partner who reads the pattern you are too close to see, and use Gemini to pressure-test whether your 'new research idea' actually follows from the feedback; 42–50 revise, ledger, submit + Claim Ticket: write the honest reflection paragraph, submit M12 tonight, and write the ticket.
...
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: your note's sharpest sentence and the claim you are least sure survives, one line each; 15–32 milestone kickoff + AI-supported sprint: the instructor presents the M15 brief (final research chapter + AI-management portfolio, staged for the multi-agent workflow next week) and draws the Evidence-Defense order for next week, then the TABLE READ runs as the work block — each note passes left, three minutes per read, a two-question margin note (sharpest sentence, weakest claim) on each, full rotation; 32–42 peer + AI red-team review: swap capsules and run your partner's restart-and-run-all cold, logging any break, then have Gemini flag any remaining inflated contribution in your discussion; 42–50 revise, ledger, submit + Claim Ticket: revise from the margin notes, submit M14 tonight, and write the ticket.
...
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: the AI decision you are proudest of overruling, one line each; 15–32 milestone kickoff + defense rounds: the instructor runs the final-dossier checklist one last time, then the REMAINING EVIDENCE DEFENSES run as the work block in the drawn order (compressed 10+5 format so all five defenders fit across the two meetings), every non-defender asking at least one ledger-grounded question; 32–42 peer + AI red-team review: a last cross-defense synthesis (what did the strongest defenses share?) and a final traceability spot-check — one number in your chapter shown to its notebook cell; 42–50 revise, ledger, submit + Claim Ticket: submit M15 and confirm it on screen, take the reflection survey, and write the final Claim Ticket.
