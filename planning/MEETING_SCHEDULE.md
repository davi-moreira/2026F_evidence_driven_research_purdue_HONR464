...
## Week 2 — Studio 2: Govern the work and declare your question
...
- **Secondary questions:** Do your four MIDA parts agree with each other? What does the M4 brief ask for (a pathway declaration defended against one assigned contrast), and how is it graded?
...
- **Concepts:** declaring a full design in public; internal-consistency check across MIDA; clarifying questions as peer review; the M4 deliverable (pathway declaration and mandated contrast); hand-off to the route hub
- **Python dependency:** none new (own milestone work)
- **Dataset / simulation:** student's own project materials
- **50-minute dynamic:** 0–10 weekly topic quiz: five multiple-choice questions on the week's topic, answered solo on paper and handed in; 10–15 research stand-up: change, blocker, decision, delegation, verification-needed from each student; 15–45 milestone kickoff from the Brightspace brief: present M4 (pathway declaration and mandated contrast) + rubric, then an AI-supported sprint — deliver 3-minute MIDA declarations, one clarifying question per listener, and use AI to probe internal consistency while logging what you accept; 45–50 revise, update the Ledger + dossier, submit M3 + Claim Ticket.
...
- **Project connection:** M3 closes today; the declared design opens the design-library phase, beginning with the M4 pathway declaration at the route hub.
...
- **SRL puzzle focus:** Puzzle: two projects share one topic (campus food insecurity) but their Contracts license different routes — name the signal that separates them.
...
- **Dataset / simulation:** lapop_brazil, cliningsmith_etal, bonilla_tillery, la_voter_file, foos_etal route cards (no new analysis; the argument is the work)
...
- **Provenance:** course studio protocol + crosswalk M4 + M4/M5 Brightspace briefs + GenAI Studio reviewer bench | weekly Friday studio | pathway-declaration assembly + required Causal Identification Skeptic review + M5 kickoff | newly-constructed-from-source-concept
- **Concepts:** the pathway declaration as a versioned commitment; the M4 checklist (route, warrant, contrast, boundary, hybrid); the required GenAI Studio 'Causal Identification Skeptic' pass; the M5 deliverable (data + measurement governance); milestone hand-off (M4 -> M5)
...
- **Milestone work time:** 30 min studio block
- **Milestone presentation/review:** M4 worked at the studio and submitted; required GenAI Studio 'Causal Identification Skeptic' review logged in the Ledger
...
- **Provenance:** course studio protocol + crosswalk M7 + GenAI Studio reviewer bench + M7/M8 Brightspace briefs | weekly Friday studio | verification-record assembly + required Prediction & Leakage Auditor review + M8 kickoff | newly-constructed-from-source-concept
...
- **Hands-on activity:** Resolve or honestly bound your final discrepancy, lock the claim-output trace, run the required Prediction & Leakage Auditor review, and submit M7.
