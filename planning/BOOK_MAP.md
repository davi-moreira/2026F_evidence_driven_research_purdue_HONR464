...
Authoritative manifest for the open-access Quarto book (D20; separation of the
book and course artifacts per D25). RDSS remains the assigned theory text; this
book is the course's own AI-era research manual. Each chapter carries the same
core elements (the research decision · conceptual explanation · a worked
example (econ / political science / business / biology) · its OWN companion
Colab notebook — `notebooks/book/chNN_<slug>.ipynb`, built by
`scripts/build_book_notebooks.py`, opened by the chapter badge — · recommended
AI prompts · a "Do not delegate" box · an AI failure case · an "It is your
turn" section that chains into a full research project across the book), and
the same undergraduate voice + citation rules as the notebooks. The **primary
notebook** column below is the course-loop mapping: each chapter's "Colab
laboratory" section names its primary course lab, and the For-instructors
appendix makes all sixteen labs available. `scripts/validate_book_sync.py`
checks, both directions, that every chapter links its companion notebook and
names its primary course notebook, that the For-instructors appendix links all
16 labs, and that every notebook nb01–nb16 is the primary of ≥1 chapter.
