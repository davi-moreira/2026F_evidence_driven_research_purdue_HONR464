...
## D65 — Davi's pass on the SRL slot announcement (2026-08-23)

The third Week-1 announcement (`_announcements/03_srl_slots_and_logistics.md`,
generated by `scripts/build_srl_packet.py`) was edited and posted. Same rule as
D62/D63/D64: his posted text is the instruction, and
`build_announcement()` was corrected until it regenerates his wording
(verified word-for-word identical). Five cuts, all adopted:

1. **Both minute-frame tables lose the "Whose block" column.** They now carry
   Minutes and Section only. The frames themselves are unchanged (D22/D34), and
   who owns which block is still taught in `project/srl/srl_handbook.md`.
2. **The instructor-intervention specifics cut.** "I will step in during your
   session, and I want you to expect it rather than read it as a rescue" stands
   alone; the three examples (flagging a spreading error, seeding a cold call,
   handing an AI failure back to the lead) and "That is the design working, not
   you struggling" are gone.
3. **The "two days ahead" item cut from the logistics list** (4 items to 3). The
   prep-template contents and the "cheapest place to fix a session" framing go
   with it. The DEADLINE itself survives twice in the announcement: "you send me
   a plan two days ahead and I send notes back", and "a prep deadline two days
   before each one".
4. **The handbook item cut from "What to do now"** (3 items to 2), taking the
   links to `srl_handbook.md`, `srl_rubric.md` and `srl_prep_template.md`.
5. **"I am sending their brief separately today, and their preparation script is
   due …" cut**, leaving "First up is <lead>, on <date>."

**No rule, date, weight or rubric row changed.** The 25 slots, the 25%, the nine
rows, the hard cap and the two-days-ahead deadline all stand.

**Why cuts 3 and 4 do not strand anyone.** Every student still receives their own
`per_student/<name>.md` in Week 1 (README: "to each student individually, Week 1,
with the announcement"), which carries the handbook, the rubric and the prep
template, and each `slot_briefs/slot_NN_*.md` carries the template again about a
week before that slot. The announcement is now the *draw plus the shape of the
